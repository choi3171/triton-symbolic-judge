...
`poison-output`, the directive for a stale-buffer read, has never fired on a natural corpus. Neither corpus has a memory FAIL, so this case exists here only as the hand-written fixtures in `tvj/fixtures/hacks.py`.
