...
In the LLM dataset the first input of every row has at most 1024 elements, and 122 of the 156 rows run every launch as a single program. The dataset was re-judged with the leading dimension enlarged, so that the flattened input spans at least two blocks and a tail for any BLOCK from 128 to 2048. Of the 107 PASS rows that could be re-judged, 104 stayed PASS and row 16 became undecided. Two are wrong at the larger shape. Row 19 divides by `factor.shape[0]`, while its reference divides by the size of the second dimension, which is the same number only because the corpus input is `[4, 4, 4, 4]`; the judge FAILs it and the tolerance test fails too. Row 153 stores each block's sum at `pid * 4` in a buffer with one slot per block, which is right only while there is one block; at the larger shape it writes past the buffer and its result changes from run to run. Three PASS rows were not re-judged: row 104, whose reference does not accept the new shape, and rows 92 and 152, whose terms are too large to judge at 33 times the size (`tvj/judge/shape2_run.py`, `python3 -m tvj.measure.shape2`). That is evidence about these kernels, not a proof for others.
