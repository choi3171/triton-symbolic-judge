...
# A single row can build tens of millions of terms (a VGG-sized pipeline with a
...
class Interp:
    ...
    def exec_block(self, ops, env):
        """Run a flat op list, following `cf.br` / `cf.cond_br` between the `^bb`
        labels the parser leaves inline.

        A program counter rather than a for-loop, because every generated kernel
        that guards itself -- `if pid >= total: return` -- lowers to a conditional
        branch over two blocks, and six trace rows were refused for it.  The
        predicate there is a function of the program id, which this interpreter
        enumerates concretely, so there is nothing symbolic to case-split: the
        branch is simply taken or not.  A branch on a value LOADED from memory is
    ...
    def _pred_bool(self, v):
        ...
        concrete program at a time, so a guard on the program id is a plain bool.
        One that depends on loaded data is not, and is refused rather than guessed."""
        x = v.data[0] if hasattr(v, "data") else v
        if isinstance(x, (bool, int)): return bool(x)
        if isinstance(x, Pred) or isinstance(x, _PredTerm) or isinstance(x, T.Term):
            raise Unsupported("branch on a value loaded from memory (cf.cond_br)")
        return bool(x)
...
_LIBDEVICE = {
    ...
    "log10f": lambda d, x: T.div(T.app("log", x), T.const(2.302585092994046)),  # ln 10
    # round-half-to-even.  Not a real-number function -- it is exactly the kind of
    # thing `select` exists for -- but it is a total function of one argument, so
    # an uninterpreted symbol keeps congruence: two sides that round the same
    # expression agree, and one that rounds a different one does not.
    "nearbyintf": lambda d, x: T.app("rint", x), "rintf": lambda d, x: T.app("rint", x),
    "roundf": lambda d, x: T.app("round", x),
    "floorf": lambda d, x: T.app("floor", x), "ceilf": lambda d, x: T.app("ceil", x),
    "truncf": lambda d, x: T.app("trunc", x),
    "isnanf": lambda d, x: T.cmp("ne", x, x),          # NaN is the only x with x != x
    "isinff": lambda d, x: T.app("isinf", x),
    "signbitf": lambda d, x: T.cmp("lt", x, T.ZERO),
    "copysignf": lambda d, x, y: T.app("copysign", x, y),
    "fmodf": lambda d, x, y: T.app("fmod", x, y),
    "sinhf": lambda d, x: T.mul(T.const(0.5), d.sub(d.exp(x), d.exp(T.mul(T.const(-1.0), x)))),
    "coshf": lambda d, x: T.mul(T.const(0.5), d.add(d.exp(x), d.exp(T.mul(T.const(-1.0), x)))),
}
