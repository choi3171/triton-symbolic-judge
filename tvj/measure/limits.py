...
# The file is chosen by tvj/judge/record.py.  This used to read whichever of
# `data/kb_live.jsonl` and the published record existed first, `data/` first -- so
# any machine that had ever run the corpus regenerated this table from its old
# scratch, silently and with exit 0.
CORPORA = [("KernelBook", "kb", 400),
           ("LLM traces", "traces", 156)]
...
if __name__ == "__main__":
    md = "--md" in sys.argv
    data = []
    for title, corpus, total in CORPORA:
        recs = load(corpus)
        if recs is None:
            sys.exit(f"limits: no run record for {title} -- looked in "
                     f"{', '.join(record.paths(corpus))}.\n"
