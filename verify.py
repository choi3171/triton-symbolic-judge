...
claim("spec_agree.py", "the spec front-end computes what torch computes: 132 cases including the full pooling flag sweep, every indirect-read spelling, `==` as a mask and multi-head attention, plus 5 modes it must refuse",
      [r"132/132 handlers agree with torch", r"masked_fill\(m == 0\).*ok", r"gather\(dim=1\).*ok", r"index_select\(dim=0\).*ok", r"5/5 refusals as expected", r"avg_pool2d k3 s2 p1 ceil=True cip=False.*ok",
...
claim("lanes.py", "one Volta call per SHAPE, not per output element: 1024 matmul lanes and 512 attention lanes are each one shape, and deciding a representative per shape gives every lane the verdict the full lane-by-lane run gives, controls included",
      [r"mm vs splitk\s+1024\s+1\s", r"mm vs bug_k\s+1024\s+1\s+0/1024",
       r"ref vs flash\s+512\s+1\s+512/512", r"ref vs norescale\s+512\s+1\s+0/512",
