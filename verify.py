...
claim("nf_rat.py", "the caps bucket is division, not multiplicative depth: on KernelBook row 61 the depth is 1, one output sums 4 fractions with distinct softmax denominators (4 softmax rows over different slices of one input), and Volta's cross-multiplied equality check has ~1e6 monomials per output before it can compare anything",
