...
case "$1" in
  kb) SCRIPT=tvj.judge.kernelbook_run; REC=data/kb_live.jsonl; LOG=results/kb_full.log
      N=$(python3 -c "import json;print(len(json.load(open('data/kernelbook_400.json'))))") ;;
  traces) SCRIPT=tvj.judge.traces_run; REC=results/triton_traces.jsonl; LOG=results/tr_full.log
      N=$(python3 -c "import json;print(sum(1 for r in json.load(open('data/triton_traces.json')) if r['source']=='kernelbook'))") ;;
...
while [ "$floor" -lt "$N" ]; do
  ...
  timeout "$CAP" python3 -u -m "$SCRIPT" "$floor" $((N - floor)) >> "$LOG" 2>&1
  next=$(( $(last) + 1 ))
